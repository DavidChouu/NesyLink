...
数据目录记录为 `nesylink/cnn/generated/button_pressed_all_v1/train`。当前仓库提交了生成器、训练程序、权重和预览，但未提交该历史生成数据目录，因此报告不声称训练集像素文件已随作业提交。生成器从公开地图、空间变体、颜色变体和运行时状态产生图像，并用内部对象状态写监督标签；内部状态仅用于离线标注。
