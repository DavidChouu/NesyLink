...
3. 下面挑选了一个组员的实验环境：实验在 WSL2 平台的 Ubuntu 22.04.5 LTS 系统中完成，使用 Intel Core i9-14900HX 处理器、15 GiB WSL2 可用内存以及 NVIDIA GeForce RTX 4060 Laptop GPU（8188 MiB 显存）。Python 实验环境由 Conda 26.1.1 管理，环境名称为 `nesylink`，使用 CPython 3.10.20、Gymnasium 1.3.0、NumPy 2.2.6、PyTorch 2.11.0+cu128 和 Torchvision 0.26.0+cu128。GPU 驱动版本为 595.97，PyTorch 使用 CUDA 12.8，运行时能够正常识别 CUDA 设备。Lean 形式化验证使用 Lean 4.32.0，并通过 Elan 4.2.3 管理固定工具链 `leanprover/lean4:v4.32.0`。实验对应的 NesyLink 项目版本为 0.1.0。
...
实验结果可见“能证明代码确实运行的截图或附件”文件夹中“python运行结果证明图”文件夹里面的9张图片。
...
| 项目 | 值 |
|:-:|:-:|
| 初始化权重 | `tiny_hybrid_cnn_button_pressed_all_v1.weights.pt` |
| 配置 epochs | 14 |
| 保存的最佳 epoch | 9 |
| batch size | 128 |
| optimizer | AdamW |
...
- 正式逐 episode JSON(即report_artifacts/robustness_suite_eval.json)
- 环境、命令、版本、哈希与验收记录(即report_artifacts/reproducibility.md)
- 472 条主要 Lean 定理清单(即report_artifacts/theorem_inventory.md)
- 环境形式化技术附录(即环境形式化说明.md)
- 策略形式化与证明技术附录(即策略形式化与证明.md)

JSON、定理清单和复现记录构成可机器检查的运行附件，满足“能证明代码确实运行的截图或附件”要求。报告不制作或伪造终端截图。
