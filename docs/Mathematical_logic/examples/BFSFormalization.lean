...
theorem bfs_first_goal_layer_is_minimal
    ...
    (hfirst : bfsFindsGoalAtDepth g start firstDepth)
...
theorem graph_path_split_at_milestone
  ...
  induction hpath with
  | nil s =>
      cases hmilestone with
      | atStart hs =>
          exact ⟨s, [], [], hs, GraphPath.nil s, GraphPath.nil s, rfl⟩
  | cons edge hedge htail ih =>
      cases hmilestone with
      | atStart hs =>
          exact ⟨_, [], edge :: _, hs, GraphPath.nil _,
            GraphPath.cons edge hedge htail, by simp⟩
      | atLater htailMilestone =>
          rcases ih htailMilestone with
            ⟨milestone, pre, post, hp, hpre, hpost, hroute⟩
          refine ⟨milestone, edge :: pre, post, hp, ?_, hpost, ?_⟩
          · exact GraphPath.cons edge hedge hpre
          · simp [hroute]
...
theorem stepwise_bfs_theorem
  ...
  · rintro ⟨milestone, goal, pre, post, hp, hpre, hgoal, hpost⟩
    exact ⟨goal, pre ++ post, hgoal,
      reachable_trans ⟨pre, hpre⟩ ⟨post, hpost⟩⟩
